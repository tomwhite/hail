  ...
  val signatures = Map(
    ...
    def run(state: State, options: Options): State = {
      ...
      if (options.store) {
        ...
        /*val newVariantAnnotations = mergedVSM.mapAnnotationsWithAggregate(new ConcordanceTable)((comb, v, s, gtp) => comb.addCount(gtp._1, gtp._2),
          (comb1, comb2) => comb1.merge(comb2), (va, table) => va.addMap("conc", table.results))
          .addSampleMapSignatures("conc", signatures)
          .addVariantMapSignatures("conc", signatures)*/
...
/*
        val newAnnotations2 = mergedVSM.rdd.map{case (v, va, gs) => (v, va.addMap("conc",variantConcordance.collectAsMap()(v).results), gs)}
        state.copy(vds = mergedVSM.mapAnnotations()



        )
        /*state.copy(vds = mergedVSM.mapAnnotationsWithAggregate(new ConcordanceTable)((comb, v, s, g) => comb.merge(g),
          (comb1, comb2) => comb1.merge(comb2),
          (ad: AnnotationData, comb: ConcordanceTable) => ad.addMap("conc", comb.asM))
          .addVariantMapSignatures("conc", VariantQCCombiner.signatures))*/*/
