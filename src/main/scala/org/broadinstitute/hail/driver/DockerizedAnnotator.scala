 ...
 */
abstract class DockerizedAnnotator extends Command with Serializable {
  ...
  /**
   * Returns a new State with the appropriate annotations  
   */
  def run(state: State, options: Options): State = {
    state.copy(vds = annotatePartitions(state.vds,options))
  }

  /**
   * Runs annotate on each partition 
   */
  def annotatePartitions(vds:VariantDataset,options:Options): VariantDataset = {
    this.options=options
    vds.copy(rdd = vds.rdd.mapPartitions(p=>{
      getWorkhorse().annotate(p)
    }))
  }
 
  trait Workhorse extends DockerRunner {
