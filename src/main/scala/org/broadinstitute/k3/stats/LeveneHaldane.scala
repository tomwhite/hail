...
import org.broadinstitute.k3.Utils.compareDouble
...
class LeveneHaldane(val n: Int,
  ...
  override def cumulativeProbability(n0: Int, n1: Int): Double =
    if (n0 >= n1 || n0 >= nA || n1 < nA % 2)
      ...
      (pLU.slice(1, (mode - n0 + 1) / 2).takeWhile(_ > 1.0E-16).sum + pRU.slice(0, (n1 - mode) / 2 + 1).takeWhile(_ > 1.0E-16).sum) / pN

  // P(nAB <= n1)
  def cumulativeProbability(n1: Int): Double = cumulativeProbability(-1, n1)
  // P(nAB > n0)
  def survivalFunction(n0: Int): Double = cumulativeProbability(n0, nA)
  ...
  // greater, lesser, or of smaller probability, respectively (in the latter case weighting outcomes of equal probability by 1/2)
  def rightMidP(nAB: Int) = survivalFunction(nAB) + probability(nAB) / 2
  def leftMidP(nAB: Int) = cumulativeProbability(nAB) - probability(nAB) / 2
  def exactMidP(nAB: Int) =
    if (nAB < 0 || nAB > nA || nAB % 2 != nA % 2)
      0.0
    else {
      val p0U = if (nAB >= mode) pRU((nAB - mode) / 2) else pLU((mode - nAB) / 2)
      val cutoff = p0U * 0.5E-16
      def mpU(s: Stream[Double]): Double = {
        val (sEq, sLess) = s.dropWhile(_ > p0U).span(compareDouble(_, p0U))
        0.5 * sEq.sum + sLess.takeWhile(_ > p0U * cutoff).sum
      }
  ...
  def getNumericalVariance: Double = 1.0 * nA * nB / (2 * n - 1) * (1 + (nA - 1.0) * (nB - 1) / (2 * n - 3) - 1.0 * nA * nB / (2 * n - 1))
...
object LeveneHaldane {
  ...
  def apply(n: Int, nA: Int, rng: RandomGenerator): LeveneHaldane = {
    ...
    def pRUfrom(nAB: Int, p: Double): Stream[Double] = p #:: pRUfrom(nAB + 2, p * (nA - nAB) * (nB - nAB) / ((nAB + 2.0) * (nAB + 1)))
    def pLUfrom(nAB: Int, p: Double): Stream[Double] = p #:: pLUfrom(nAB - 2, p * nAB * (nAB - 1) / ((nA - nAB + 2.0) * (nB - nAB + 2)))
